...
void Drive::pid_wait_timeout_set(int timeout_ms) { default_pid_wait_timeout = timeout_ms; }
int Drive::pid_wait_timeout_get() { return default_pid_wait_timeout; }

// User wrapper for exit condition
void Drive::pid_wait(int timeout_ms) {
  // Use global default if no timeout specified
  int active_timeout = timeout_ms > 0 ? timeout_ms : default_pid_wait_timeout;
  uint32_t start_time = pros::millis();

  // Helper lambda: returns true if the timeout has been reached
  auto timed_out = [&]() -> bool {
    if (active_timeout <= 0) return false;
    return (int)(pros::millis() - start_time) >= active_timeout;
  };
  ...
  if (mode == DRIVE) {
    ...
    while ((left_exit == RUNNING || right_exit == RUNNING) && !timed_out()) {
    ...
    if (timed_out()) {
      if (print_toggle) printf("  Drive: Timeout Exit (%d ms)\n", active_timeout);
    } else {
  ...
  // Odom Exits
    ...
    if (mode == PURE_PURSUIT) {
      while (pp_index != pp_movements.size() - 1 && !timed_out()) {
    ...
    while ((xy_exit == RUNNING || a_exit == RUNNING) && !timed_out()) {
      xyPID.velocity_sensor_secondary_set(drive_imu_accel_get());
      current_a_odomPID.velocity_sensor_secondary_set(drive_imu_accel_get());
      xy_exit = xy_exit != RUNNING ? xy_exit : xyPID.exit_condition({left_motors[0], right_motors[0]});
      a_exit = a_exit != RUNNING ? a_exit : current_a_odomPID.exit_condition({left_motors[0], right_motors[0]});
    ...
    if (timed_out()) {
      if (print_toggle) printf("  Odom: Timeout Exit (%d ms)\n", active_timeout);
    } else {
  ...
  // Turn Then Drive Exit
  else if (mode == TURN_THEN_DRIVE) {
    // Wait for the turn phase to complete (handled by the task)
    while (!ttd_turn_complete && !timed_out()) {
      turnPID.velocity_sensor_secondary_set(drive_imu_accel_get());
      exit_output turn_exit = turnPID.exit_condition({left_motors[0], right_motors[0]});
      if (turn_exit != RUNNING) break;
      pros::delay(util::DELAY_TIME);
    }

    // Wait for the drive phase to complete using odom exits
    exit_output xy_exit = RUNNING;
    exit_output a_exit = RUNNING;
    while ((xy_exit == RUNNING || a_exit == RUNNING) && !timed_out()) {
      xyPID.velocity_sensor_secondary_set(drive_imu_accel_get());
    ...
    if (timed_out()) {
      if (print_toggle) printf("  Turn Then Drive: Timeout Exit (%d ms)\n", active_timeout);
    } else {
      if (print_toggle) std::cout << "  Turn Then Drive - XY: " << exit_to_string(xy_exit) << " Exit, error: " << xyPID.error << ".   Angle: " << exit_to_string(a_exit) << " Exit, error: " << current_a_odomPID.error << ".\n";
    }
  ...
  // Swing Exit
    ...
    pros::Motor& sensor = current_swing == vortex::LEFT_SWING ? left_motors[0] : right_motors[0];
    while (swing_exit == RUNNING && !timed_out()) {
    ...
    if (timed_out()) {
      if (print_toggle) printf("  Swing: Timeout Exit (%d ms)\n", active_timeout);
    } else {
