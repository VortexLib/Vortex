...
std::pair<float, float> Drive::decide_vert_sensor(vortex::tracking_wheel* tracker, bool is_tracker_enabled, float ime, float ime_track) {
  float current = ime;
...
vortex::pose Drive::solve_xy_vert(float p_track_width, float current_t, float delta_vert, float delta_t) {
...
vortex::pose Drive::solve_xy_horiz(float p_track_width, float current_t, float delta_horiz, float delta_t) {
  pose output = {0.0, 0.0, 0.0};
...
void Drive::ez_tracking_task() {
  ...
  vortex::tracking_wheel* h_sensor = odom_tracker_back != nullptr ? odom_tracker_back : odom_tracker_front;
  ...
  // Angle and velocity
  float t_current = -vortex::util::to_rad(drive_imu_get());  // negative for math standard
