...
bool Drive::is_swing_slew_enabled(e_swing type, double target, double current) {
  int side = type == vortex::LEFT_SWING ? 1 : -1;
...
void Drive::pid_swing_behavior_set(vortex::e_angle_behavior behavior) { default_swing_type = behavior; }
vortex::e_angle_behavior Drive::pid_swing_behavior_get() { return default_swing_type; }
...
void Drive::pid_swing_set(e_swing type, double target, int speed, int opposite_speed, e_angle_behavior behavior, bool slew_on) {
  ...
  if (odom_theta_direction_get())
    current_swing = current_swing == vortex::LEFT_SWING ? vortex::RIGHT_SWING : vortex::LEFT_SWING;
