...
void Drive::pid_odom_behavior_set(vortex::e_angle_behavior behavior) { default_odom_type = behavior; }
vortex::e_angle_behavior Drive::pid_odom_behavior_get() { return default_odom_type; }
...
/////
// No units
void Drive::pid_odom_injected_pp_set(std::vector<vortex::odom> imovements) {
  bool slew_on = imovements[0].drive_direction == fwd ? slew_drive_forward_get() : slew_drive_backward_get();
  pid_odom_injected_pp_set(imovements, slew_on);
}
void Drive::pid_odom_injected_pp_set(std::vector<vortex::odom> imovements, bool slew_on) {
  xyPID.timers_reset();
  current_a_odomPID.timers_reset();
...
// Units
void Drive::pid_odom_injected_pp_set(std::vector<vortex::united_odom> p_imovements) {
  std::vector<odom> imovements = util::united_odoms_to_odoms(p_imovements);
  pid_odom_injected_pp_set(imovements);
}
void Drive::pid_odom_injected_pp_set(std::vector<vortex::united_odom> p_imovements, bool slew_on) {
  std::vector<odom> imovements = util::united_odoms_to_odoms(p_imovements);
  pid_odom_injected_pp_set(imovements, slew_on);
}
...
/////
// No units
void Drive::pid_odom_turn_then_drive_set(odom imovement) {
  bool slew_on = imovement.drive_direction == fwd ? slew_drive_forward_get() : slew_drive_backward_get();
  pid_odom_turn_then_drive_set(imovement, slew_on);
}
void Drive::pid_odom_turn_then_drive_set(odom imovement, bool slew_on) {
  // Store the original (unflipped) movement so pid_odom_ptp_set can flip it later
  ttd_movement = imovement;
  ttd_slew_on = slew_on;
  ttd_turn_complete = false;

  // Flip for calculating the turn angle
  odom flipped = set_odom_direction(imovement);

  // Calculate angle to target point
  pose current = odom_pose_get();
  double angle = util::absolute_angle_to_point({flipped.target.x, flipped.target.y}, current);

  // If reversed, face 180 degrees away from the target
  if (flipped.drive_direction == REV) {
    angle += 180.0;
  }

  if (print_toggle) printf("Turn Then Drive Started... Target: (%.2f, %.2f)  Turn Target: %.2f\n", flipped.target.x, flipped.target.y, angle);

  // Start the turn phase
  pid_turn_set(angle, imovement.max_xy_speed, pid_turn_behavior_get(), slew_turn_get());
  drive_mode_set(TURN_THEN_DRIVE);
}
// Units
void Drive::pid_odom_turn_then_drive_set(united_odom p_imovement) {
  odom imovement = util::united_odom_to_odom(p_imovement);
  pid_odom_turn_then_drive_set(imovement);
}
void Drive::pid_odom_turn_then_drive_set(united_odom p_imovement, bool slew_on) {
  odom imovement = util::united_odom_to_odom(p_imovement);
  pid_odom_turn_then_drive_set(imovement, slew_on);
}
