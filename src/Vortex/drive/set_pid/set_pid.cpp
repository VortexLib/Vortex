...
void Drive::pid_angle_behavior_bias_set(e_angle_behavior behavior) {
  if (behavior == vortex::LEFT_TURN)
    turn_biased_left = true;
  else if (behavior == vortex::RIGHT_TURN)
    turn_biased_left = false;
  else
    printf("Must input 'left' or 'right' for angle behavior bias!\n");
}
e_angle_behavior Drive::pid_angle_behavior_bias_get() { return turn_biased_left ? vortex::LEFT_TURN : vortex::RIGHT_TURN; }
...
//  - longest
//  - raw
void Drive::pid_angle_behavior_set(vortex::e_angle_behavior behavior) {
