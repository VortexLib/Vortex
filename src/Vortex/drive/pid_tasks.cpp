...
// Turn Then Drive task
// Phase 1: turn to face the target (or 180 away if reversed)
// Phase 2: once turn settles, drive straight to the target using PTP
void Drive::turn_then_drive_task() {
  if (!ttd_turn_complete) {
    // Run the turn PID
    turn_pid_task();

    // Check if the turn has settled
    exit_output turn_exit = turnPID.exit_condition({left_motors[0], right_motors[0]});
    if (turn_exit != RUNNING) {
      ttd_turn_complete = true;

      if (print_toggle) printf("  Turn phase complete, starting drive to (%.2f, %.2f)\n", ttd_movement.target.x, ttd_movement.target.y);

      // Transition to PTP drive phase
      pid_odom_ptp_set(ttd_movement, ttd_slew_on);
      drive_mode_set(TURN_THEN_DRIVE);  // Override mode back since ptp_set changes it
    }
  } else {
    // Run the PTP drive task
    ptp_task();
  }
}