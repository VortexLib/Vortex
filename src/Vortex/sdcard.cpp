...
void auto_sd_update() {
  ...
  if (!vortex::util::SD_CARD_ACTIVE) return;
...
void auton_selector_initialize() {
  ...
  if ((as_usd_file_read = fopen("/usd/auto.txt", "r"))) {
    ...
    vortex::as::auton_selector.auton_page_current = std::stof(a_buf);
  ...
  if (vortex::as::auton_selector.auton_page_current > vortex::as::auton_selector.auton_count - 1 || vortex::as::auton_selector.auton_page_current < 0) {
    vortex::as::auton_selector.auton_page_current = 0;
    vortex::as::auto_sd_update();
  }
}
...
void initialize() {
  ...
  pros::lcd::initialize();
  vortex::as::auton_selector_initialize();

  // Callbacks for auto selector
  print_page();
  pros::lcd::register_btn0_cb(vortex::as::page_down);
  pros::lcd::register_btn2_cb(vortex::as::page_up);
...
pros::Task limit_switch_task(vortex::as::limitSwitchTask);
