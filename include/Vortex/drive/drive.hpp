...
#include "Vortex/slew.hpp"
#include "Vortex/tracking_wheel.hpp"
...
class Drive {
 public:
   ...
   * Slew objects.
   */
  vortex::slew slew_left;
  vortex::slew slew_right;
  vortex::slew slew_forward;
  vortex::slew slew_backward;
  vortex::slew slew_turn;
  vortex::slew slew_swing_forward;
  vortex::slew slew_swing_backward;
  vortex::slew slew_swing;
   ...
   *
   * \param behavior
   *        vortex::shortest, vortex::longest, vortex::left, vortex::right, vortex::raw
   */
   ...
   *
   * \param stick_type
   *        vortex::SINGLE or vortex::SPLIT control
   */
  ...
  /**
   * Turns to face the target point, then drives in a straight line to it.
   * If the direction is rev, the robot turns 180 degrees away from the target and drives backwards.
   * Uses slew if globally enabled.
   *
   * \param imovement
   *        {{x, y}, fwd/rev, 1-127}  an odom movement
   */
  void pid_odom_turn_then_drive_set(odom imovement);
   ...
   *        ramp up from a lower speed to your target speed
   */
  void pid_odom_turn_then_drive_set(odom imovement, bool slew_on);
   ...
   *
   * \param imovement
   *        {{x, y}, fwd/rev, 1-127}  an odom movement.  values are united here with okapi units
   */
  void pid_odom_turn_then_drive_set(united_odom p_imovement);
   ...
   *        ramp up from a lower speed to your target speed
   */
  void pid_odom_turn_then_drive_set(united_odom p_imovement, bool slew_on);
   ...
   *
   * \param timeout_ms
   *        maximum time in milliseconds to wait before moving on.  0 = no timeout (default).
   *        if a global default timeout is set, passing 0 will use that instead.
   */
  void pid_wait(int timeout_ms = 0);

  /**
   * Set a global default timeout for pid_wait in milliseconds.
   * This is used when pid_wait is called with timeout_ms = 0.
   * Set to 0 to disable the global default timeout.
   *
   * \param timeout_ms
   *        default timeout in milliseconds.  0 = no timeout.
   */
  void pid_wait_timeout_set(int timeout_ms);

  /**
   * Get the current global default timeout for pid_wait.
   *
   * \return
   *        timeout in milliseconds, 0 means no timeout
   */
  int pid_wait_timeout_get();
 ...
 private:
  ...
  double new_turn_target_compute(double target, double current, vortex::e_angle_behavior behavior);
  ...
  // Turn-then-drive state
  odom ttd_movement = {};
  bool ttd_slew_on = false;
  bool ttd_turn_complete = false;
  void turn_then_drive_task();

  // Global default timeout for pid_wait (0 = disabled)
  int default_pid_wait_timeout = 0;
  ...
  std::pair<float, float> decide_vert_sensor(vortex::tracking_wheel* tracker, bool is_tracker_enabled, float ime = 0.0, float ime_track = 0.0);
...
#define ODOM_TRACKER 4
  ...
  /////
  //
  // Timed (Blocking) Motion Overloads
  //
  // Each of these calls the corresponding non-blocking setter, then pid_wait with the given timeout.
  // Usage: chassis.pid_drive_set(24_in, DRIVE_SPEED, 2000_ms);
  //
  /////

 public:
  // --- pid_drive_set ---
  void pid_drive_set(okapi::QLength p_target, int speed, okapi::QTime timeout);
  void pid_drive_set(okapi::QLength p_target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_drive_set(double target, int speed, okapi::QTime timeout);
  void pid_drive_set(double target, int speed, bool slew_on, okapi::QTime timeout);

  // --- pid_turn_set (pose) ---
  void pid_turn_set(pose itarget, drive_directions dir, int speed, okapi::QTime timeout);
  void pid_turn_set(pose itarget, drive_directions dir, int speed, bool slew_on, okapi::QTime timeout);
  void pid_turn_set(pose itarget, drive_directions dir, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_turn_set(pose itarget, drive_directions dir, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_turn_set (united_pose) ---
  void pid_turn_set(united_pose p_itarget, drive_directions dir, int speed, okapi::QTime timeout);
  void pid_turn_set(united_pose p_itarget, drive_directions dir, int speed, bool slew_on, okapi::QTime timeout);
  void pid_turn_set(united_pose p_itarget, drive_directions dir, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_turn_set(united_pose p_itarget, drive_directions dir, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_turn_set (double) ---
  void pid_turn_set(double target, int speed, okapi::QTime timeout);
  void pid_turn_set(double target, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_turn_set(double target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_turn_set(double target, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_turn_set (QAngle) ---
  void pid_turn_set(okapi::QAngle p_target, int speed, okapi::QTime timeout);
  void pid_turn_set(okapi::QAngle p_target, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_turn_set(okapi::QAngle p_target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_turn_set(okapi::QAngle p_target, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_turn_relative_set (QAngle) ---
  void pid_turn_relative_set(okapi::QAngle p_target, int speed, okapi::QTime timeout);
  void pid_turn_relative_set(okapi::QAngle p_target, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_turn_relative_set(okapi::QAngle p_target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_turn_relative_set(okapi::QAngle p_target, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_turn_relative_set (double) ---
  void pid_turn_relative_set(double target, int speed, okapi::QTime timeout);
  void pid_turn_relative_set(double target, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_turn_relative_set(double target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_turn_relative_set(double target, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_swing_set (double) ---
  void pid_swing_set(e_swing type, double target, int speed, okapi::QTime timeout);
  void pid_swing_set(e_swing type, double target, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_swing_set(e_swing type, double target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_swing_set(e_swing type, double target, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);
  void pid_swing_set(e_swing type, double target, int speed, int opposite_speed, okapi::QTime timeout);
  void pid_swing_set(e_swing type, double target, int speed, int opposite_speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_swing_set(e_swing type, double target, int speed, int opposite_speed, bool slew_on, okapi::QTime timeout);
  void pid_swing_set(e_swing type, double target, int speed, int opposite_speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_swing_set (QAngle) ---
  void pid_swing_set(e_swing type, okapi::QAngle p_target, int speed, okapi::QTime timeout);
  void pid_swing_set(e_swing type, okapi::QAngle p_target, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_swing_set(e_swing type, okapi::QAngle p_target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_swing_set(e_swing type, okapi::QAngle p_target, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);
  void pid_swing_set(e_swing type, okapi::QAngle p_target, int speed, int opposite_speed, okapi::QTime timeout);
  void pid_swing_set(e_swing type, okapi::QAngle p_target, int speed, int opposite_speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_swing_set(e_swing type, okapi::QAngle p_target, int speed, int opposite_speed, bool slew_on, okapi::QTime timeout);
  void pid_swing_set(e_swing type, okapi::QAngle p_target, int speed, int opposite_speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_swing_relative_set (QAngle) ---
  void pid_swing_relative_set(e_swing type, okapi::QAngle p_target, int speed, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, okapi::QAngle p_target, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, okapi::QAngle p_target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, okapi::QAngle p_target, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, okapi::QAngle p_target, int speed, int opposite_speed, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, okapi::QAngle p_target, int speed, int opposite_speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, okapi::QAngle p_target, int speed, int opposite_speed, bool slew_on, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, okapi::QAngle p_target, int speed, int opposite_speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_swing_relative_set (double) ---
  void pid_swing_relative_set(e_swing type, double target, int speed, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, double target, int speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, double target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, double target, int speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, double target, int speed, int opposite_speed, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, double target, int speed, int opposite_speed, e_angle_behavior behavior, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, double target, int speed, int opposite_speed, bool slew_on, okapi::QTime timeout);
  void pid_swing_relative_set(e_swing type, double target, int speed, int opposite_speed, e_angle_behavior behavior, bool slew_on, okapi::QTime timeout);

  // --- pid_odom_set ---
  void pid_odom_set(double target, int speed, okapi::QTime timeout);
  void pid_odom_set(double target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_odom_set(okapi::QLength p_target, int speed, okapi::QTime timeout);
  void pid_odom_set(okapi::QLength p_target, int speed, bool slew_on, okapi::QTime timeout);
  void pid_odom_set(odom imovement, okapi::QTime timeout);
  void pid_odom_set(odom imovement, bool slew_on, okapi::QTime timeout);
  void pid_odom_set(united_odom p_imovement, okapi::QTime timeout);
  void pid_odom_set(united_odom p_imovement, bool slew_on, okapi::QTime timeout);
  void pid_odom_set(std::vector<odom> imovements, okapi::QTime timeout);
  void pid_odom_set(std::vector<odom> imovements, bool slew_on, okapi::QTime timeout);
  void pid_odom_set(std::vector<united_odom> p_imovements, okapi::QTime timeout);
  void pid_odom_set(std::vector<united_odom> p_imovements, bool slew_on, okapi::QTime timeout);

  // --- pid_odom_ptp_set ---
  void pid_odom_ptp_set(odom imovement, okapi::QTime timeout);
  void pid_odom_ptp_set(odom imovement, bool slew_on, okapi::QTime timeout);
  void pid_odom_ptp_set(united_odom p_imovement, okapi::QTime timeout);
  void pid_odom_ptp_set(united_odom p_imovement, bool slew_on, okapi::QTime timeout);

  // --- pid_odom_boomerang_set ---
  void pid_odom_boomerang_set(odom imovement, okapi::QTime timeout);
  void pid_odom_boomerang_set(odom imovement, bool slew_on, okapi::QTime timeout);
  void pid_odom_boomerang_set(united_odom p_imovement, okapi::QTime timeout);
  void pid_odom_boomerang_set(united_odom p_imovement, bool slew_on, okapi::QTime timeout);

  // --- pid_odom_pp_set ---
  void pid_odom_pp_set(std::vector<odom> imovements, okapi::QTime timeout);
  void pid_odom_pp_set(std::vector<odom> imovements, bool slew_on, okapi::QTime timeout);
  void pid_odom_pp_set(std::vector<united_odom> p_imovements, okapi::QTime timeout);
  void pid_odom_pp_set(std::vector<united_odom> p_imovements, bool slew_on, okapi::QTime timeout);

  // --- pid_odom_injected_pp_set ---
  void pid_odom_injected_pp_set(std::vector<odom> imovements, okapi::QTime timeout);
  void pid_odom_injected_pp_set(std::vector<odom> imovements, bool slew_on, okapi::QTime timeout);
  void pid_odom_injected_pp_set(std::vector<united_odom> p_imovements, okapi::QTime timeout);
  void pid_odom_injected_pp_set(std::vector<united_odom> p_imovements, bool slew_on, okapi::QTime timeout);

  // --- pid_odom_smooth_pp_set ---
  void pid_odom_smooth_pp_set(std::vector<odom> imovements, okapi::QTime timeout);
  void pid_odom_smooth_pp_set(std::vector<odom> imovements, bool slew_on, okapi::QTime timeout);
  void pid_odom_smooth_pp_set(std::vector<united_odom> p_imovements, okapi::QTime timeout);
  void pid_odom_smooth_pp_set(std::vector<united_odom> p_imovements, bool slew_on, okapi::QTime timeout);

  // --- pid_odom_turn_then_drive_set ---
  void pid_odom_turn_then_drive_set(odom imovement, okapi::QTime timeout);
  void pid_odom_turn_then_drive_set(odom imovement, bool slew_on, okapi::QTime timeout);
  void pid_odom_turn_then_drive_set(united_odom p_imovement, okapi::QTime timeout);
  void pid_odom_turn_then_drive_set(united_odom p_imovement, bool slew_on, okapi::QTime timeout);
};
};  // namespace vortex
