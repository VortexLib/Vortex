...
vortex::ControllerSettings linear(
   ...
   500,   // largeErrorTimeout — ms to hold within largeError before exit
    20,   // slew — max power change per 10 ms loop
   127,   // maxVelocity — max motor power (0–127), default 127
     0    // maxAccel — max acceleration per loop (0 = unlimited)
);
```
